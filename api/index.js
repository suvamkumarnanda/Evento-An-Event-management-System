...
app.patch("/updateEvent",  async (req, res) => {
   ...
   try {
      ...
      // eventData.image = req.file ? req.file.path : "";
         ...
         const newEvent=await Event.updateOne({title},req.body);
         // await newEvent.save();  
      res.status(201).json(newEvent);
      }
       catch (error) {
      res.status(500).json({ error: "Failed to update the event to MongoDB" });
   }
});
...
app.delete("/deleteEvent",  async (req, res) => {
   ...
   try {
      ...
      // eventData.image = req.file ? req.file.path : "";
         ...
         const newEvent=await Event.deleteOne({title});
         // await newEvent.save();  
      res.status(201).json(newEvent);
      }
       catch (error) {
      res.status(500).json({ error: "Failed to delete the event to MongoDB" });
   }
});
